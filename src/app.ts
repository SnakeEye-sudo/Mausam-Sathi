(() => {
  ...
  function render() {
    ...
    tempText.textContent = `${Math.round(state.weather.current.temperature_2m)}°C`;
    weatherDesc.textContent = codeLabel(state.weather.current.weather_code);
    feelText.textContent = `${Math.round(state.weather.current.apparent_temperature)}°C`;
  ...
  async function loadForecast(lat, lon, label) {
    ...
    state.hourlyNote = `Updated ${new Intl.DateTimeFormat("en-IN", { hour: "numeric", minute: "2-digit" }).format(new Date())}`;
    render();
  }
