...
(() => {
  ...
  `;
    ...
    function render() {
      ...
      tempText.textContent = `${Math.round(current.temperature_2m)}\xB0C`;
      weatherDesc.textContent = codeLabel(current.weather_code);
      feelText.textContent = `${Math.round(current.apparent_temperature)}\xB0C`;
      ...
      outsideMood.textContent = current.weather_code <= 2 ? "Bahar ka mahaul clean aur open feel de raha hai." : current.weather_code >= 61 ? "Bahar halka ya strong wet mood me hai, outing plan check kar lo." : "Mausam mixed hai, sky aur air dono shift me hain.";
    ...
    `;
      ...
      <div class="history-card">
        ...
        <span class="muted">${Math.round(state.weather.hourly.temperature_2m[index])}\xB0C \u2022 ${codeLabel(state.weather.hourly.weather_code[index])}</span>
      </div>
    `).join("");
    ...
    async function loadForecast(lat, lon, label) {
      ...
      state.label = label;
      state.hourlyText = `Updated ${new Intl.DateTimeFormat("en-IN", { hour: "numeric", minute: "2-digit" }).format(/* @__PURE__ */ new Date())}`;
      render();
    }
